...
void ClosestHit(inout RayPayload rayPayload : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
{
    ...
    float3 reflectDir = reflect(rayDir, inputData.normalWS);
     ...
     if(rayPayload.data & ONLY_REFLECTIONRAY_FLAG == 1)
     {
         #ifdef _REFLECTION_SCREEN
         half3 color = RayReflectionCalc(inputData.positionWS, reflectDir, rayPayload, false);
         if (color.x < 0.h)
         {
    ...
    if(surfaceData.alpha < 1.h && rayPayload.depth < gMaxTransparentDepth)
    {
        half3 transColor = RayTransparentCalc(inputData.positionWS, rayDir, rayPayload);

        if (transColor.x < 0.h)
        {
            if (_REFLECTION_PROBE_BLENDING)
            {
                transColor = CalculateIrradianceFromReflectionProbes(rayDir, worldPos, 0, half2(1.f, 1.f));
            }
            else
            {
                half4 encodedIrradiance = half4(SAMPLE_TEXTURECUBE_LOD(unity_SpecCube0, samplerunity_SpecCube0, rayDir, 0));

                transColor = DecodeHDREnvironment(encodedIrradiance, unity_SpecCube0_HDR);
            }
        }

        rayPayload.color = lerp(rayPayload.color, transColor, surfaceData.alpha);
    }
#endif

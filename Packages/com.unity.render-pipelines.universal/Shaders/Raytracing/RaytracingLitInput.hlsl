...
#include "Packages/com.unity.render-pipelines.universal/Shaders/Raytracing/RaytracingSurfaceInput.hlsl"
...
TEXTURE2D(_SubsurfaceColorMap); SAMPLER(sampler_SubsurfaceColorMap);
    ...
    #define SAMPLE_METALLICSPECULAR(uv, lod) SampleTex2D(_SpecGlossMap, sampler_SpecGlossMap, uv, lod)
#else
    #define SAMPLE_METALLICSPECULAR(uv, lod) SampleTex2D(_MetallicGlossMap, sampler_MetallicGlossMap, uv, lod)
#endif

half4 SampleMetallicSpecGloss(float2 uv, half albedoAlpha, float lod)
{
    half4 specGloss;
...
half SampleOcclusion(float2 uv, float lod)
{
    #ifdef _OCCLUSIONMAP
        half occ = SampleTex2D(_OcclusionMap, sampler_OcclusionMap, uv, lod).g;
...
// rgb == color
// a == scale
half4 SampleSubsurfaceColor(float2 uv, float lod)
{
#if defined(_SUBSURFACECOLOR) || defined(_SUBSURFACEMAP)
    half4 subsurfaceData = _SubsurfaceColor;

#if defined(_SUBSURFACEMAP)
    subsurfaceData *= SampleTex2D(_SubsurfaceColorMap, sampler_SubsurfaceColorMap, uv, lod).rgba;
#endif
...
// .x/.r == mask
// .y/.g == smoothness
half2 SampleClearCoat(float2 uv, float lod)
{
#if defined(_CLEARCOAT) || defined(_CLEARCOATMAP)
    half2 clearCoatMaskSmoothness = half2(_ClearCoatMask, _ClearCoatSmoothness);

#if defined(_CLEARCOATMAP)
    clearCoatMaskSmoothness *= SampleTex2D(_ClearCoatMap, sampler_ClearCoatMap, uv, lod).rg;
#endif
...
void ApplyPerPixelDisplacement(half3 viewDirTS, inout float2 uv, float lod)
{
...
half3 ScaleDetailAlbedo(half3 detailAlbedo, half scale, float lod)
{
...
half3 ApplyDetailAlbedo(float2 detailUv, half3 albedo, half detailMask, float lod)
{
#if defined(_DETAIL)
    half3 detailAlbedo = SampleTex2D(_DetailAlbedoMap, sampler_DetailAlbedoMap, detailUv, lod).rgb;
...
half3 ApplyDetailNormal(float2 detailUv, half3 normalTS, half detailMask, float lod)
{
#if defined(_DETAIL)
#if BUMP_SCALE_NOT_SUPPORTED
    half3 detailNormalTS = UnpackNormal(SampleTex2D(_DetailNormalMap, sampler_DetailNormalMap, detailUv, lod));
#else
    half3 detailNormalTS = UnpackNormalScale(SampleTex2D(_DetailNormalMap, sampler_DetailNormalMap, detailUv, lod), _DetailNormalMapScale);
#endif
...
inline void InitializePlasticLitSurfaceData(float2 uv, float lod, out SurfaceData outSurfaceData)
{
    half4 albedoAlpha = SampleAlbedoAlpha(uv, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap), lod);
    outSurfaceData.alpha = Alpha(albedoAlpha.a, _BaseColor, _Cutoff);

    half4 specGloss = SampleMetallicSpecGloss(uv, albedoAlpha.a, lod);
    ...
    outSurfaceData.normalTS = SampleNormal(uv, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap), lod, _BumpScale);
    outSurfaceData.occlusion = SampleOcclusion(uv, lod);
    outSurfaceData.emission = SampleEmission(uv, _EmissionColor.rgb, TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap), lod);

#if defined(_CLEARCOAT) || defined(_CLEARCOATMAP)
    half2 clearCoat = SampleClearCoat(uv, lod);
...
#if defined(_SUBSURFACECOLOR) || defined(_SUBSURFACEMAP)
    half4 subsurfaceColor = SampleSubsurfaceColor(uv, lod);
...
#if defined(_DETAIL)
    half detailMask = SampleTex2D(_DetailMask, sampler_DetailMask, uv, lod).a;
